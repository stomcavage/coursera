...
vector_matrix_product_1 = [1, 0]       # [0.5, 0.5] + [0.5, -0.5] 
vector_matrix_product_2 = [0, 4.44]    # [0, 0] + [0, 4.44]
vector_matrix_product_3 = [14, 20, 26] # [1,2,3] + [4,6,8] + [9,12,15]
...
M_swap_two_vector = [[0, 1], [1, 0]] # [[a, c], [b, d]] * [x, y] = [y, x];  find M



## Problem 3
# Represent with a list of rows lists.
three_by_three_matrix = [[1, 0, 1], [0, 1, 0], [1, 0, 0]] # M * [x, y, z] = [z + x, y, x]; find M


## Problem 4
# Represent with a list of row lists.
multiplied_matrix = [[2, 0, 0], [0, 4, 0], [0, 0, 3]] # M * [x, y, z] = [2x, 4y, 3z]
...
part_4_valid = True
part_4_number_rows = 1
part_4_number_cols = 2

part_5_valid = True
part_5_number_rows = 1
part_5_number_cols = 3
...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    colVecs = matutil.mat2coldict(M)
    return sum(v[d] * colVecs[d] for d in v.D)
...
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    rowVecs = matutil.mat2rowdict(M)
    return sum(v[d] * rowVecs[d] for d in v.D)
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    rowVecs = matutil.mat2rowdict(M)
    return Vec(M.D[0], {key : value * v for (key,value) in rowVecs.items() })
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    colVecs = matutil.mat2coldict(M)
    return Vec(M.D[1], {key : value * v for (key,value) in colVecs.items() })
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    colVecs = matutil.mat2coldict(B)
    return matutil.coldict2mat({ key : A * value for (key, value) in colVecs.items() })
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    rowVecs = matutil.mat2rowdict(A)
    return matutil.rowdict2mat({ key : value * B for (key, value) in rowVecs.items() })
...
def dot_prod_mat_mat_mult(A, B):
    ...
    rowVecs = matutil.mat2rowdict(A)
    colVecs = matutil.mat2coldict(B)
    return Mat((A.D[0], B.D[1]), { (k1, k2) : v1 * v2 for (k1, v1) in rowVecs.items() for (k2, v2) in colVecs.items() })
