...
def vec_select(veclist, k): 
    ...
    True
    '''
    return [ v for v in veclist if k not in v.f.keys() or v[k] == 0 ] 
...
def vec_sum(veclist, D): 
    ...
    True
    '''
    return Vec( D, { k:v for (k,v) in [ (d, sum([ u[d] for u in veclist ])) for d in D ] if v != 0} )
...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [ Vec(v.D, { d : (v[d] / k) for d in v.D }) for (k,v) in vecdict.items() ]
